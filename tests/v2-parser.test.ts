...
test("parses the real full-image classification schema generated by the V2 writer", () => {
  ...
  assert.equal(generatedRoot.project.projectFiles[0]?.className, undefined);
  ...
  assert.deepEqual(reparsed.project.files[0]?.labels[0]?.geometry.box, {
    x: 0,
    y: 0,
    width: 32,
    height: 24,
  });
  assert.equal(reparsed.project.files[0]?.labels[0]?.geometry.contours?.length, 1);
  assert.equal(
...
test("parses the native V2 2.7.8 classification golden without blocking", () => {
  const jsonText = readFileSync(
    new URL("../work/native-export/golden.subvisionproj", import.meta.url),
    "utf8",
  );
  const result = parseV2SubvisionProject({
  ...
  if (!result.ok) return;
  assert.equal(result.compatibility.status, "confirmation-required");
  assert.equal(result.project.files.length, 312);
